...
const StatewiseExportDetails = () => {
  ...
  useEffect(() => {
    if (selectedYear) {
      axios
        .get(
          `analysis/foodShareByContinent?commodity=${selectedCommodity}&state=${selectedState}&startYear=${selectedYear}&endYear=${selectedYear}&type=export`
        )
        .then(importResponse => {
          /* 
      ...
      <div style={{ margin: '10% 25% 10% 25%' }}>
        <Doughnut
          ...
          options={{
            title: {
              display: true,
              text: 'Statewise export details in $(USD)',
              fontSize: 20
            },
            legend: {
              display: true,
