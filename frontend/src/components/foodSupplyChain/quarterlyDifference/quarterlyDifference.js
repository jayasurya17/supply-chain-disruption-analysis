...
class QuarterlyDifference extends Component {
    ...
    async componentDidMount() {
        ...
        this.updateGraph(commoditiesResponse.data[0], unitsResponse.data[0], stateResponse.data[0])
    ...
    onSelectState = async (e) => {
        ...
        this.updateGraph(this.state.selectedCommodity, this.state.selectedUnit, e.target.value)
    }

    updateGraph = async (commodity, unit, state) => {
        let tableResponse = await axios.get(`/analysis/covidFoodProductionDisruptionByState?commodity=${commodity}&unit=${unit}&state=${state}`)
        this.setState({
        ...
        return (
            <div className="m-2">
                ...
                <div className='row'>
                    ...
                    <div className='col-md-3'>
                        State
                        <select onChange={this.onSelectState} className="form-control" name="state" value={this.state.selectedState} placeholder="State">
                            {allStates}
                        </select>
                    </div>
                </div>
