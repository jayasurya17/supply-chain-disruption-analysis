...
class CropsPerState extends Component {
    ...
    async componentDidMount() {
        ...
        let commoditiesResponse = await axios.get(`/filters/commoditiesByCategory?category=${categoriesResponse.data[0]}`)
        this.setState({
            allCommodities: commoditiesResponse.data,
            selectedCommodity: commoditiesResponse.data[0]
        })

        let unitsResponse = await axios.get(`/filters/units?category=${categoriesResponse.data[0]}&commodity=${commoditiesResponse.data[0]}`)
        this.setState({
            allUnits: unitsResponse.data,
            selectedUnit: unitsResponse.data[0]
        })

        this.updateGraph(commoditiesResponse.data[0], unitsResponse.data[0], 1980, 2020)
    ...
    onSelectCategory = async (e) => {
        ...
        let commoditiesResponse = await axios.get(`/filters/commoditiesByCategory?category=${value}`)
        this.setState({
            allCommodities: commoditiesResponse.data,
            selectedCommodity: commoditiesResponse.data[0]
        })

        let unitsResponse = await axios.get(`/filters/units?category=${value}&commodity=${commoditiesResponse.data[0]}`)
        this.setState({
            allUnits: unitsResponse.data,
            selectedUnit: unitsResponse.data[0]
        })

        this.updateGraph(commoditiesResponse.data[0], unitsResponse.data[0], this.state.year.min, this.state.year.max)
    }
    ...
    onSelectCommodity = async (e) => {
        ...
        let unitsResponse = await axios.get(`/filters/units?category=${this.state.selectedCategory}&commodity=${value}`)
        this.setState({
            allUnits: unitsResponse.data,
            selectedUnit: unitsResponse.data[0]
        })

        this.updateGraph(value, unitsResponse.data[0], this.state.year.min, this.state.year.max)
    }
    ...
    onSelectUnit = (e) => {
        ...
        this.updateGraph(this.state.selectedCommodity, e.target.value, this.state.year.min, this.state.year.max)
    }

    updateGraph = async (commodity, unit, startYear, endYear) => {
        let mapResponse = await axios.get(`/analysis/foodProductionByState?commodity=${commodity}&unit=${unit}&startYear=${startYear}&endYear=${endYear}`)
        this.setState({
        ...
        return (
            <div>
                ...
                <div className='row mt-3 ml-5 mr-5'>
                    ...
                    <div className='col-md-4'>
                        Commodity
                        <select onChange={this.onSelectCommodity} className="form-control" name="commodity" value={this.state.selectedCommodity} placeholder="Commodity">
                            {allCommodities}
                        </select>
                    </div>
                    <div className='col-md-4'>
                        Unit
                        <select onChange={this.onSelectUnit} className="form-control" name="unit" value={this.state.selectedUnit} placeholder="Unit">
                            {allUnits}
                        </select>
                    </div>
                </div>
